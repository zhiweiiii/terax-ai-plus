...
- **Shared PTY**: Web viewers subscribe to the same `Arc<Session>` as the desktop (`Session::web_subscribe`), so input and output are identical on both ends. The flusher thread keeps a rolling output history (`WEB_HISTORY_CAP` = 256 KiB) for late viewers and broadcasts every chunk to attached Web clients. **The desktop owns the canonical PTY size** — the phone never sends a resize and renders at **exactly the PTY grid** (both buffers): the byte stream is laid out against the desktop's cols/rows (app wrapping, `\r` redraws, absolute cursor moves), so re-wrapping to the phone width garbles lines. Wide grids scroll horizontally in the container; tall grids overflow vertically (normal buffer scrolls, alt screen clips the bottom). The server sends the `attached` size **before** the history replay so history bytes parse at the right grid. Each connection subscribes with its own `SyncSender` and disconnect removes exactly that subscription; a viewer that falls behind is evicted and told to resubscribe.
