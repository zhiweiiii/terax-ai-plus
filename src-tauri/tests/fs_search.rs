...
use terax_lib::modules::fs::tree::{fs_read_dir_impl, list_subdirs_impl, EntryKind};
...
fn grep_case_insensitive_finds_mixed_case() {
    ...
    let strict = fs_grep_impl("hello".into(), fx.root_str(), None, Some(false), None, None)
        .expect("grep");
    assert!(strict.hits.is_empty());

    let loose = fs_grep_impl("hello".into(), fx.root_str(), None, Some(true), None, None)
        .expect("grep");
...
fn grep_max_results_truncates() {
    ...
    let res = fs_grep_impl("needle".into(), fx.root_str(), None, None, Some(3), None)
        .expect("grep");
...
fn grep_empty_pattern_errors() {
    let fx = FsFixture::new();
    let err = fs_grep_impl("".into(), fx.root_str(), None, None, None, None);
    assert!(err.is_err());
}
...
fn grep_respects_ignore_file() {
    ...
    let res = fs_grep_impl("secret".into(), fx.root_str(), None, None, None, None)
        .expect("grep");
...
fn glob_finds_files_by_pattern() {
    ...
    let res = fs_glob_impl("**/*.rs".into(), fx.root_str(), None, None).expect("glob");
...
fn glob_truncates_on_limit() {
    ...
    let res = fs_glob_impl("*.txt".into(), fx.root_str(), Some(5), None).expect("glob");
...
fn glob_empty_pattern_errors() {
    let fx = FsFixture::new();
    assert!(fs_glob_impl("".into(), fx.root_str(), None, None).is_err());
}
...
fn search_substring_matches_filename() {
    ...
    let res = fs_search_impl(fx.root_str(), "main".into(), None, None, None).expect("search");
    let rels: Vec<&str> = res.hits.iter().map(|h| h.rel.as_str()).collect();
...
fn search_is_case_insensitive() {
    let fx = FsFixture::new();
    fx.write("README.md", "");
    let res = fs_search_impl(fx.root_str(), "readme".into(), None, None, None).expect("search");
    assert_eq!(res.hits.len(), 1);
}
...
fn search_empty_query_returns_empty() {
    let fx = FsFixture::new();
    fx.write("a.txt", "");
    let res = fs_search_impl(fx.root_str(), "   ".into(), None, None, None).expect("search");
...
fn search_prunes_node_modules() {
    ...
    let res = fs_search_impl(fx.root_str(), "index".into(), None, None, None).expect("search");
    let rels: Vec<&str> = res.hits.iter().map(|h| h.rel.as_str()).collect();
...
fn search_ranks_filename_hits_before_path_hits() {
    ...
    let res = fs_search_impl(fx.root_str(), "zeta".into(), None, None, None).expect("search");
    let zeta_file = res
        .hits
        .iter()
...
fn list_files_returns_sorted_relative_paths() {
    ...
    let res = fs_list_files_impl(fx.root_str(), None, None, None, None).expect("list");
...
fn list_files_max_depth_clamps() {
    ...
    let res = fs_list_files_impl(fx.root_str(), None, Some(1), None, None).expect("list");
...
fn list_files_non_dir_errors() {
    assert!(fs_list_files_impl("/no/such/dir".into(), None, None, None, None).is_err());
}
...
fn read_dir_orders_dirs_before_files_then_alpha() {
    ...
    let entries = fs_read_dir_impl(fx.root_str(), false, None, None).expect("read_dir");
...
fn read_dir_hides_dotfiles_by_default() {
    ...
    let hidden_off = fs_read_dir_impl(fx.root_str(), false, None, None).expect("read_dir");
    let names: Vec<&str> = hidden_off.iter().map(|e| e.name.as_str()).collect();
    assert_eq!(names, vec!["visible.txt"]);

    let hidden_on = fs_read_dir_impl(fx.root_str(), true, None, None).expect("read_dir");
...
fn read_dir_flags_gitignored_entries_only_when_requested() {
    ...
    let entries = fs_read_dir_impl(fx.repo_str(), false, Some(true), None).expect("read_dir");
    let flag = |name: &str| {
        entries
            .iter()
            .find(|e| e.name == name)
    ...
    let plain = fs_read_dir_impl(fx.repo_str(), false, None, None).expect("read_dir");
...
fn read_dir_skips_gitignore_outside_a_repo() {
    ...
    fx.write("kept.txt", "");
    let entries = fs_read_dir_impl(fx.root_str(), false, Some(true), None).expect("read_dir");
...
fn list_subdirs_returns_only_directories() {
    ...
    let dirs = list_subdirs_impl(fx.root_str(), false, None).expect("list_subdirs");
...
fn list_subdirs_hides_dot_dirs_by_default() {
    ...
    let off = list_subdirs_impl(fx.root_str(), false, None).expect("list_subdirs");
    assert_eq!(off, vec!["visible"]);

    let on = list_subdirs_impl(fx.root_str(), true, None).expect("list_subdirs");
