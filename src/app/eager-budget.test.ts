...
// Locks the startup-bundle invariant: the heavy editor / markdown stacks must
// stay out of the eager graph of both window entries so they load only when
// the user opens those surfaces. A static import that re-introduces any of
// these (e.g. a barrel re-export, or a `cn`-style util getting absorbed into a
// feature chunk) will fail here. xterm and motion are intentionally eager
// (terminal-first shell) and are not asserted against.
